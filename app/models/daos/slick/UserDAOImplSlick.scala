 ...
 */
class UserDAOImplSlick @Inject() (protected val dbConfigProvider: DatabaseConfigProvider) extends UserDAO with DAOSlick {
  ...
  def save(user: User) = {
    ...
    Logger.info("avatart url dans save = " + user.avatarURL)
    ...
    val actions = (for {
      ...
      _ <- slickUserLoginInfos += DBUserLoginInfo(None, dbUser.userID, loginInfo.id.get)
      _ <- slickOpenIDInfos += DBOpenIDInfo(dbUser.userID, loginInfo.id.get)
